...
export class AppError extends Error {
  ...
  constructor(codeOrType: number | ErrorType, message?: string, data?: object, cause?: unknown) {
    const code = typeof codeOrType === 'object' ? codeOrType.code : codeOrType;
    const msg =
      typeof codeOrType === 'object' ? (message !== undefined ? message : codeOrType.msg) : (message as string);
    super(msg);
    this.name = 'AppError';
    this.code = code;
    this.data = data;
